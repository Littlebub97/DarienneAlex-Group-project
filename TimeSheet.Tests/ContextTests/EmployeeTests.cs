...
namespace TimeSheet.Test.ContextTests
{
    ...
    public class EmployeeTests : IDisposable
    {
        ...
        public void Dispose()
        {
         ...
         {
          _db.Database.ExecuteSqlCommand("Delete from TimeSheet.TimeSheet");
            _db.Database.ExecuteSqlCommand($"DBCC CHECKIDENT (\"TimeSheet.TimeSheet\", RESEED, -1);");
          }
        ...
        {
            var employee = new Employee { EmployeeName = "Whatever", lastName = "Bar", firstName = "foo", address = "123 lane", exempt = true, Password = "1234" };
        ...
        {
            _db.Employees.Add(new Employee { EmployeeName = "Foo", lastName = "Bar", firstName = "foo", address = "123 lane", exempt = true, Password = "1234" });
            _db.Employees.Add(new Employee { EmployeeName = "Bar", lastName = "Foo", firstName = "foo", address = "123 lane", exempt = true, Password = "1234" });
            _db.SaveChanges();
            var Employees = _db.Employees.OrderBy(c => c.lastName).ToList();
           // Assert.Equal(2, _db.Employees.Count());
          //  Assert.Equal("Bar", Employees[0].lastName);
           // Assert.Equal("Foo", Employees[1].lastName);
        }

        [Fact]
        public void ShouldUpdateAnEmployee()
        {
            var Employee = new Employee { EmployeeName = "Foo", lastName = "Bar", firstName = "foo", address = "123 lane", exempt = true, Password = "1234"  };
            _db.Employees.Add(Employee);
            _db.SaveChanges();
